...
class PrettyPrinter:
	...
	def visitNumber(self, number, sp_cnt):
		print(indent_type*sp_cnt + str(number.value), end="")

	def visitFunctionDefinition(self, foo, sp_cnt):
		print(indent_type*sp_cnt + "def " + foo.name + " (" + ", ".join(foo.function.args) + ") {")
		for cmnd in foo.function.body:
			self.visit(cmnd, sp_cnt+1)
			print(";")
		print(indent_type*sp_cnt+"}", end="")

	def visitConditional(self, conditional, sp_cnt):
		print(indent_type*sp_cnt + "if (", end="")
	...
	def visitPrint(self, prnt, sp_cnt):
		print(indent_type*sp_cnt + "print(", end="")
		self.visit(prnt.expr, 0)
		print(")", end="")

	def visitRead(self, read, sp_cnt):
		print(indent_type*sp_cnt + "read(" + read.name + ")", end="")
	...
	def visitReference(self, reference, sp_cnt):
		print(indent_type*sp_cnt + reference.name, end="")
